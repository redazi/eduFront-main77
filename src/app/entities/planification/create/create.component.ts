...
calendarOptions: CalendarOptions|undefined;
  ...
  */ 
  ...
  this.panierService.getplanifbyres(res.event?.id).subscribe(data=>{

    const planification1 = data;
    
    console.log("lplanification id hiya : "+planification1);
    this.panierService.checkIfDejaExistist(planification1).subscribe(data=>{

      this.paniers = data;
        console.log("hana hnaa db lpanier fih :"+this.paniers.length);
       if(this.paniers.length>0){
       console.log("dejaa exist ");
       this.openDialog5('500ms','500ms');
